	...
	if (patientID != "") {
		...
		pt.read()
			.then(function (p) {
				...
				$("#patientInfo").html("<b>Patient Info</b><br /><div id='patInfo'><div class='labl'>Name:</div><div>"  + p.name[0].given + " " +  p.name[0].family + "</div><br />" + "<div class='labl'>Sex:</div><div>" + gender + "</div><br />" + "<div class='labl'>Born:</div> <div>" + p.birthDate + "</div><br />" + "<div class='labl'>Age:</div> <div id='ageInfo'>" + getAge(p.birthDate) + "</div><br />" + "<div class='labl'>Active:</div> <div>" + p.active + "</div></div>");
				$('#patientInfo').css('height', '160px');
				$('#exp').attr('src','images/chevron-expand.png');
				...
				$('#ageInfo').html($('#adultAge').html()+$('#pediAge').html());
...
var appID = urlParam("app");
 ...
 try {
	FHIR.oauth2.ready(function(smart){
		console.log(smart.context.patient);
		patientID = smart.patientId;
		console.log("OAuth Patient ID: " + patientID);
	});
} catch (err) {
	console.log("Unable to verify oAuth context: " + err);
}
*/
...
$(window).load(function(){
	...
	try {
		if (urlParam("patients") != null) {
			...
			FHIR.oauth2.ready(function(smart){
				//console.log(smart.context.patient);
				patientIDs.push(smart.patientId);
