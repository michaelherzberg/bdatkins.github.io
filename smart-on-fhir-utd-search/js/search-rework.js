...
$.utd.init({
	...
	tabId: 'searchTab',
	onReadyUrl: 'https://acdswww.utdlab.com/services/AutoComplete',
	showSurvey:
	false,
	locale: 'en',
	 removeTxt: 'Remove',
	exportPowerpoint: true
	 ...
	 userEngagement:
		true,
	acceptUrl: 'https://acdswww.utdlab.com/services/AgreementWebService?type=accept',
	declineUrl: 'https://acdswww.utdlab.com/home/contact-us',
	licenseUrl: 'https://acdswww.utdlab.com/contents/license',
	marketoUrl: 'https://acdswww.utdlab.com/compliance/info',
	cancelComplianceUrl: 'https://acdswww.utdlab.com/compliance/info?method=cancelled',
	pwUrl: 'https://acdswww.utdlab.com/account/contact?method=cancel',
...
//var fhirServiceUrl = "http://smartfhir01d.utd.com:9080";
//var fhirServiceUrl = "https://fhir-open-api.smarthealthit.org";
//var fhirServiceUrl = "http://utd02808.utd.com:9081";
...
function urlParam(p) {
  ...
  if (result.length === 0){
	...
	$('#exp').attr('src', 'images/box-arrow-up.png');
	$("#patientInfo").html("<b>No Patient Info</b>");
	$('input:radio[name="sp"][value="0"]').prop('checked', true);
	$('.styledRadio:eq(0)').css("backgroundPosition", "0px -16px");

	return null;
  } else {
	$('#patientInfo').css('height', 'auto');
	$('#exp').attr('src','images/box-arrow-down.png');
  }
  return result[0];
}
...
function fetchApp(appId) {
	...
	$.get(
		...
		$("#patientInfo").append("<b style='color:red'>Unable to obtain app info</b>");
		$('#patientEx').hide();
	  });
...
function fetchPatientName(patientID) {
	...
	if (patientID != "") {
		$.get(
			...
			$('#patientEx').hide();
			$("#patientInfo").append("<div class='err'><div>ERROR: Unable to load patient data.</div><div>Host:<br /> "+ fhirServiceUrl + "</div><div>Patient ID: " + patientID + "</div></div>");
			$('input:radio[name="sp"][value="0"]').prop('checked', true);
	  });
	}
 ...
 function fetchPatientBirthday(patientID) {
	...
	$.get(
		...
		$("#patientInfo").append("<b style='color:red'>No Patient Birthday</b>");
	  });
...
function fetchPatientGender(patientID) {
	...
	$.get(
		...
		$("#patientInfo").append("<b style='color:red'>No Patient Gender</b>");
	  });
	...
	function onChange () {
		...
		pt.read()
			.then(function (p) {
				...
				$('.styledRadio').css("backgroundPosition", "0px 0px");
				...
				$("#patientInfo").html("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Patient Info:</b><div id='patInfo'><div class='labl'>Name:</div><div class='val'>"  + p.name[0].given + " " +  p.name[0].family + "</div>" + "<div class='labl'>Sex:</div><div class='val'>" + p.gender.coding[0].display + "</div>" + "<div class='labl'>Born:</div> <div class='val'>" + p.birthDate + "</div>" + "<div class='labl'>Age:</div> <div id='ageInfo' class='val'>" + friendlyAge + "</div></div>");
	...
	if (objSmart != null) {
		...
		pt.read()
			.then(function (p) {
				...
				$("#patientInfo").html("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Patient Info:</b><br /><div id='patInfo'><div class='labl'>Name:</div><div class='val'>"  + p.name[0].given + " " +  p.name[0].family + "</div>" + "<div class='labl'>Sex:</div><div class='val'>" + gender + "</div>" + "<div class='labl'>Born:</div> <div class='val'>" + p.birthDate + "</div>" + "<div class='labl'>Age:</div> <div id='ageInfo' class='val'>" + friendlyAge + "</div></div>");
				$('#patientInfo').css('height', 'auto');
				$('#exp').attr('src','images/box-arrow-down.png');
...
$(window).load(function(){
	...
	try {
		if (urlParam("patients") != null) {
			...
			// failover fhir server
			fhirServiceUrl = "http://smartfhir01d.utd.com:9080";
	...
	$('input:radio').screwDefaultButtons({
		image: 'url("images/radio_btns.png")',
		width: 16,
		height: 16
	});
